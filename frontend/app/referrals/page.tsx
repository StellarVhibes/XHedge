...
export default function ReferralsPage() {
  ...
  const referralCode = address ? address.slice(-8).toUpperCase() : "XXXX";
  const referralLink = `https://xhedge.fi/ref/${referralCode}`;
...
function RewardRow({ address, activity, reward, date }: RewardRowProps) {
  return (
    <tr className="hover:bg-muted/30 transition-colors">
      <td className="px-6 py-4 font-mono text-muted-foreground">{address}</td>
