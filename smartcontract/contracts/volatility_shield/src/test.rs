...
fn test_take_fees() {
    ...
    // fee_percentage defaults to 0 after simplified init;
    // manually set it via storage helper
...
fn test_rebalance_admin_auth_accepted() {
    ...
    // Oracle is not set; but rebalance with empty allocations should pass
    // since require_admin_or_oracle checks Admin key (which is now set)
