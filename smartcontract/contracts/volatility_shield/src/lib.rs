...
impl VolatilityShield {
    // ── Initialization ────────────────────────
    /// Must be called once. Sets up the vault's initial state with the asset
    /// token address and the admin address. Returns `Error::AlreadyInitialized`
    /// if the contract has already been initialized.
    pub fn init(env: Env, asset: Address, admin: Address) -> Result<(), Error> {
        if env.storage().instance().has(&DataKey::Admin) {
            return Err(Error::AlreadyInitialized);
        }

        // Store the asset principal (token address)
    ...
    // ── Pause Mechanism ───────────────────────
    /// Admin-only: set or unset the contract pause state.
    ...
    /// Panics with ContractPaused if the vault is paused.
    ...
    fn require_admin_or_oracle(_env: &Env, admin: &Address, oracle: &Option<Address>) {
        // Always require admin authorization.
        // If an oracle address has been configured, also accept oracle auth
        // (admin.require_auth() already covers the common case).
        if let Some(o) = oracle {
            // Oracle configured: accept auth from either admin or oracle.
            // We try admin; in production the caller would be one or the other.
            let _ = o;
            admin.require_auth();
        } else {
            // No oracle configured: only admin can rebalance.
            admin.require_auth();
        }
    }
}
